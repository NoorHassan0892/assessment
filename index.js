...
function isMenuItemInCategory(menuItem, category) {
  // Your code here
  const result = menu.find((name1) => {
    if (category == menu.category) return true;
  });
}
...
return menu.ingredient.length } else { false }
...
function listMenuItemNamesByCategory(menu, category) {
  // Your code here
  const result =[]
  const menu1=menu.find((category)=>{
